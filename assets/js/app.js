document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!menuToggle) {
    console.error('Mobile menu toggle button (.mobile-menu-toggle) not found.');
  }
  if (!mainNav) {
    console.error('Main navigation container (.main-navigation) not found.');
  }
  ...
  playButtons.forEach(button => {
    ...
    if (button.getAttribute('href') === '/play') {
      button.addEventListener('click', function (event) {
        ...
        if (!baseReferralUrl) {
          console.error('Play button clicked, but referralBaseUrl is empty.');
        ...
        console.log(`Redirecting to (already includes sub_id3): ${baseReferralUrl}`);
